import { useStore } from '../store';
import { Avatar } from './ui';
import { IconBell } from './Icons';
import { Logo } from './Brand';

// The mark itself lives in Brand.tsx, which serves the real artwork. Re-exported here
// so the screens that already import `Logo` from this file keep working.
export { Logo, BrandLockup, BrandMark } from './Brand';

export function AppHeader() {
  const { state, dispatch } = useStore();
  const unreadNotifications = state.notifications.filter((n) => n.unread).length;
  return (
    <header className="flex h-12 shrink-0 items-center justify-between px-5">
      <Logo />
      <div className="flex items-center gap-3">
        <button
          onClick={() => dispatch({ type: 'nav', screen: 'notifications' })}
          className="press relative text-ink-500"
          aria-label="Notifications"
        >
          <IconBell size={20} />
          {unreadNotifications > 0 && (
            <span className="absolute -right-0.5 -top-0.5 h-2 w-2 rounded-full bg-primary-500 ring-2 ring-canvas" />
          )}
        </button>
        <button
          onClick={() => dispatch({ type: 'tab', tab: 'profile' })}
          className="press"
          aria-label="Profile"
        >
          <Avatar initial={state.student.initial} src={state.student.avatarUrl} size={32} />
        </button>
      </div>
    </header>
  );
}
